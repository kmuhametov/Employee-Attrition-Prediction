...
1. **Clone the repository**
   ```bash
   git clone https://github.com/kmuhametov/Employee-Attrition-Prediction
...
5. **Start the FastAPI server**
   ```bash
   uv run uvicorn src.serve:app --reload --port 9696
...
2. **Run the container**
   ```bash
   docker run -it --rm -p 9696:9696 attrition_prediction
